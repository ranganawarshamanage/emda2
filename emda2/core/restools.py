...
def get_resolution_array(uc, hf1):
    ...
    if nx == ny == nz:
        nbin, res_arr, bin_idx, sgrid = fcodes2.resol_grid_em(
            uc, debug_mode, maxbin, nx, ny, nz
        )
    else:
        nbin, res_arr, bin_idx, sgrid = fcodes2.resolution_grid(
            uc, debug_mode, maxbin, nx, ny, nz
        )
    return nbin, res_arr[:nbin], bin_idx, sgrid
...
def create_kernel(fResArr, smax):
    # Create kernel. smax is resolution
    ...
    print("boxsize: ", box_size, "radius: ", box_radius, "center:", center)
    radius = box_radius
...
def cut_resolution(f, bin_idx, cbin):
    """fcodes2.cutmap cuts f according to resolution defined by smax
    and output same size map as f but padded with zeros outside."""
    nx, ny, nz = f.shape
    nbin = np.max(bin_idx) + 1
    fout = fcodes2.cutmap(f, bin_idx, cbin, 0, nbin, nx, ny, nz)
    return fout


def cut_resolution_for_linefit(f, bin_idx, res_arr, smax):
    """Trimmed array of FCs"""
    nx, ny, nz = f.shape
    cbin = cx = smax
    dx = int((nx - 2 * cx) / 2)
    dy = int((ny - 2 * cx) / 2)
    ...
    fout = fcodes2.cutmap(f, bin_idx, cbin, 0, len(res_arr), nx, ny, nz)
    fout = fout[dx : dx + 2 * cx, dy : dy + 2 * cx, dz : dz + 2 * cx]
    return fout, cBIdx, cbin


def cut_resolution_nmaps(f_list, bin_idx, res_arr, smax):
    """Trimmed arrays of FCs"""
