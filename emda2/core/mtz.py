...
from emda2.config import debug_mode
...
class Mtz:
    ...
    def to_f3d(self, mapsize=None):
        ...
        self.f3d = fcodes2.mtz2_3d(self.h,
                                   self.k,
                                   self.l,
                                   self.f1d,
                                   nz, ny, nx,
...
def write_3d2mtz(unit_cell, mapdata, outfile="emda_map2mtz.mtz", resol=None):
    ...
    if resol is None:
        resol = 0.0
    """ nbin, res_arr, bin_idx, sgrid = restools.get_resolution_array(unit_cell, mapdata)
    if resol is None:
        cbin = nbin
    else:
    ...
    """ h, k, l, ampli, phase = fcodes2.prepare_hkl(
        mapdata, bin_idx, cbin, 1, nx, ny, nz) """
    h,k,l,ampli,phase,jrange = fcodes2.prepare_hkl3(unit_cell,mapdata,resol,debug_mode,nx,ny,nz)
    h = h[:jrange[1]]
    k = k[:jrange[1]]
    l = l[:jrange[1]]
    ampli = ampli[:jrange[1]]
    phase = phase[:jrange[1]]
