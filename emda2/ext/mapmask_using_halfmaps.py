...
def create_mapmask_islandlabelling2(rho1, rho1avg, thresh):
    ...
    blobs_labels, nlabels = measure.label(
        blobs, background=0, connectivity=blobs.ndim, return_num=True
    )
    ...
    sblob_area, sblob_number = sort_together(
        [blob_area, blob_number], reverse=True
    )
    bnum_highvol = []
    bsum_highvol = []
    rmsd_list = []
    ...
    print("sorting...")
    srmsd_list, sbnum_highvol = sort_together(
        [rmsd_list, bnum_highvol], reverse=True
    )
...
def find_vf(arr, plotname="plot.png"):
    ...
    xc, yc = rdp_algo.run_rdp(
        np.asarray(threshlist), np.asarray(vflist), epsilon=0.005
    )
    fig = plt.figure()
...
def cdf_thresholding(arr, prob=0.99, plotname="cdf.png", vthresh=0.0):
    import matplotlib
    ...
    ax2.plot(
        (X2[0], X2[-1]), (F2[ibin], F2[ibin]), color="black", linestyle=":"
    )
...
def main(h1, h2, emdbid):
    ...
    halffsc = em.fsc(f1=f1, f2=f2, bin_idx=bin_idx, nbin=nbin)
    ...
    fscweights_halfmap = fcodes2.read_into_grid(
        bin_idx, fsc_str_halfmap, nbin, nx, ny, nz
    )
