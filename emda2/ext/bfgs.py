...
from emda2.core import fsctools, quaternions
import warnings
# from emda2.ext.overlay import get_avg_fsc
from emda2.ext.utils import get_avg_fsc
...
class MinimizeStopper(object):
    ...
    def __call__(self, xk):
        ...
        if self.nit > 10:
            warnings.warn(
                "Terminating optimization: time limit reached", TookTooLong
            )
        else:
...
class Bfgs:
    def __init__(self):
        ...
        self.t = np.array([0.0, 0.0, 0.0], "float")
        self.xyz = None
        self.xyz_sum = None
        self.bin_idx = None
        self.nbin = None
    ...
    def hess_r(self, x):
        ...
        q = np.array([1.0, 0.0, 0.0, 0.0], "float") + np.insert(step, 0, 0.0)
        ...
        binfsc, _, _ = fsctools.anytwomaps_fsc_covariance(
    ...
    def derivatives(self, x):
        ...
        dRdq = quaternions.derivatives_wrt_q(self.q)
        # print('dRdq')
        # print(dRdq)
    ...
    def calc_fsc(self):
        ...
        binfsc = np.where(~mask, binfsc, 0.0)
        self.binfsc = binfsc
    ...
    def get_wght(self):
        ...
        self.wgrid = fcodes2.read_into_grid2(
            self.bin_idx, val_arr, self.nbin, nz, ny, nx
        )[:, :, :, 0]
    ...
    def functional(self, x):
        ...
        print(
            "fval, rot(deg), trans(A): {:6.5f} {:6.4f} {:6.4f}".format(
    ...
    def optimize(self):
        ...
        print(
            "Euler angles: ",
            np.rad2deg(
                quaternions.rotationMatrixToEulerAngles(
                    quaternions.get_RM(self.q)
                )
            ),
        )
    ...
    def derivatives(self, x):
        ...
        for i in range(3):
            df[i] = (
                np.sum(
                    np.real(
                        wgrid * np.conjugate(self.e0) * (ert * tpi * sv[i])
                    )
                )
                / vol
            )
