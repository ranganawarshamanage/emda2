...
def decide_pointgroup(axeslist, orderlist):
    ...
    # check is orderlist is empty
    if len(orderlist) == 0:
        print("orderlist is empty.")
        return [pg]
    ...
    if len(axeslist) == 2:
        ...
        if abs(angle - 63.47) <= ang_tol or abs(angle - 116.57) <= ang_tol:
            ...
            if min(orderlist) == 2 and max(orderlist) == 3:
                pg = "O"
        elif abs(angle - 109.47) <= ang_tol or abs(angle - 70.53) <= ang_tol:
            assert max(orderlist) == 3
            assert min(orderlist) == 3
            pg = "T"
        elif abs(angle - 54.74) <= ang_tol or abs(angle - 125.26) <= ang_tol:
            if max(orderlist) == 3 and min(orderlist) == 2:
                pg = "T"
            if max(orderlist) == 4 and min(orderlist) == 3:
                pg = "O"
        elif angle < ang_tol or (180.0 - angle) < ang_tol:
    ...
    if len(axeslist) > 2:
        orderlist, axeslist = sort_together(
            [orderlist, axeslist], reverse=True
        )
        ...
        if len(ax3) > 0 and len(ax2) > 0:
            for axis1 in ax3:
                for axis2 in ax2:
                    ...
                    if (
                        abs(angle - 54.74) <= ang_tol
                        or abs(angle - 125.26) <= ang_tol
                    ):
                        pg = "T"
                        return [pg]
                    else:
        ...
        for i, ax in enumerate(axeslist[1:]):
            ...
            if angle < ang_tol or (180.0 - angle) < ang_tol:
                mask.append(True)
            else:
                mask.append(False)
        if all(mask):
