...
refined_t = [0., 0., 0.]  # default translation
pg = None
...
def is_given_order_correct(emmap1, axis, t, known_order, current_order, fobj):
    ...
    ang = float(
        360 * (angledif_list.index(min(angledif_list)) + 1) / L)
    ...
    emmap1.symdat.append([[current_order],axis,list(fsc_results[0]), list(t)])
...
def get_axorder(emmap1, refined_axis, order_list, fobj=None, t=None):
    ...
    try:
        ...
        for order in order_list:
            ...
            fsc_results = calc_fsc(emmap1=emmap1,
                           axis=refined_axis,
                ...
                if fsc > fsc_thresh_for_trueorder:
                    ...
                    while fsc > fsc_thresh_for_trueorder:
                        ...
                        if current_odr1 > 50:
                            emdalogger.log_string(
                                fobj,
                                'Current order is greater than 50. Stop.')
                            break
                        porder = previous_odr1
                        corder = current_odr1
                        angledif_list = [
                            abs(float(360/porder/2) - 
                            float(360*j/corder)) for j in range(1, corder)]
                        ang = float(
                            360 * (angledif_list.index(min(angledif_list)) + 1) / 
                            current_odr1)
                        fsc_results = calc_fsc(
                            emmap1=emmap1,
                            axis=refined_axis,
                        ...
                        if fsc_results is not None:
                            fsc = fsc_results[1]
                            emmap1.symdat.append([[corder],refined_axis,list(fsc_results[0]), list(t)])
                        else:
                            emdalogger.log_string(
                                fobj,
                                'Something went wrong in FSC calculation. Stop'
                            )
                            break
                        emdalogger.log_string(
                            fobj,
    ...
    try:
        if len(true_order_list) == 0:
            ...
            for odr in new_order_list:
                angledif_list = [
                    abs(float(360/maxorder/2) - 
                    float(360*j/odr)) for j in range(1, odr)]
                ang = float(
                ...
                fsc = fsc_results[1]
                emmap1.symdat.append([[odr],refined_axis,list(fsc_results[0]), list(t)])
                emdalogger.log_string(
                    fobj,
                    '   Axis: %s   Order: %s   FSC: % .3f' %
...
def refine_ax(emmap1, axlist, orderlist, fobj):
    try:
        ...
        for i, ax in enumerate(axlist):
            ...
            emdalogger.log_string(
                fobj,
                "FSCs before and after axis refinement"
            )
            emdalogger.log_fsc(
                fobj,
                {
...
def check_tetrahedral(emmap1, tetrahedral_2axes, tetrahedral_2axes_fsc, fobj):
    if len(tetrahedral_2axes) > 0:
        ...
        th_axis = tetrahedral_2axes[np.argmax(
...
def check_octahedral(emmap1, octahedral_2axes, octahedral_2axes_fsc, fobj):
    if len(octahedral_2axes) > 0:
        ...
        # refine the 2-fold with highest FSC
        oc_axis = octahedral_2axes[np.argmax(
...
def five_folds(emmap1, axes, sordern, fscs, fobj):
    ...
    if len(sorder5ax) > 1:
        try:
            ...
            emdalogger.log_string(
                fobj,
                '   Angle between them: % .3f' % angle
            )
            if (abs(angle - 63.47) <= ang_tol_p or abs(angle - 116.57) <= ang_tol_p):
                # ax1, ax2 refinement
                refinement_results = refine_ax(
                    emmap1=emmap1,
                ...
                if refinement_results is not None:
                    ...
                    if fsc_rax1 >= pg_decide_fsc and fsc_rax2 >= pg_decide_fsc:
                        ...
                        if abs(angle - 63.47) <= ang_tol or abs(angle - 116.57) <= ang_tol:
                            pg = 'I'
                    elif fsc_rax1 >= pg_decide_fsc and fsc_rax2 < pg_decide_fsc:
    ...
    try:
        ...
        if refinement_results is not None:
            ...
            if fsc_refined_5fold > pg_decide_fsc:
                emdalogger.log_string(
                    fobj,
                    '5-fold is real. Checking for D or C...' 
                )
                ...
                if len(sorder3ax) > 0:
                    for i, ax in enumerate(sorder3ax):
                        emdalogger.log_string(
                            fobj,
                            '   Axis1: %s   Order: %s   FSC: % .3f' %
                            (vec2string(refined_main_axis), 5, refined_mainax_fsc)
                        )
                        emdalogger.log_string(
                            fobj,
                            '   Axis2: %s   Order: %s   FSC: % .3f' %
                ...
                if len(parellel_orders) > 1:
                    emdalogger.log_string(
                        fobj,
                        'parellel_orders: %s'% parellel_orders 
                    )
                    """ result = [
                ...
                if mainax_bestorder_fsc >= pg_decide_fsc:
                    ...
                    if len(perpendicular_axes) > 0:
                        ...
                        sorted_perpendicular_axes) = sort_together(
                            [perpendicular_fscs, perpendicular_axes], reverse=True)
                        refinement_results = refine_ax(
                            emmap1=emmap1,
        ...
        try:
            ...
            for i, ax in enumerate(sorder3ax):
                if i > 0:
                    ...
                    if abs(angle - 109.47) <= ang_tol_p or abs(angle - 70.53) <= ang_tol_p:
            ...
            if len(temp_3ax_list) > 0:
                ...
                if refinement_results is not None:
                    (ref_axlist,
                     ...
                     _,_,
                     ref_fsclist) = refinement_results
                    refined_ax1_o3, refined_ax2_o3 = ref_axlist[0], ref_axlist[1]
                    fsc_refined_ax1, fsc_refined_ax2 = ref_fsclist[0], ref_fsclist[1]
                    if fsc_refined_ax1 >= pg_decide_fsc and fsc_refined_ax2 >= pg_decide_fsc:
                        # both 3-fld axes are valid
                        angle = cosine_angle(refined_ax1_o3, refined_ax2_o3)
                        if abs(angle - 109.47) <= ang_tol or abs(angle - 70.53) <= ang_tol:
                            ...
                            if len(sorder2ax) > 0:
                                ...
                                emdalogger.log_string(
                                    fobj, 'They have neccessory relationship for O/T')
                                emdalogger.log_string(
                            ...
                            pg = single3fold(
                                ...
                                sordern=sordern, 
                                fobj=fobj)
                    elif fsc_refined_ax1 >= pg_decide_fsc and fsc_refined_ax2 < pg_decide_fsc:
            ...
            if fsc_mainax_refined > pg_decide_fsc:
                ...
                if len(sorder2ax) > 0:
                    for i, ax in enumerate(sorder2ax):
                        ...
                        if abs(angle - 54.74) <= ang_tol_p or abs(angle - 125.26) <= ang_tol_p:
                            # tetrahedral angle
                            tetrahedral_2axes.append(ax)
                            tetrahedral_2axes_fsc.append(sorder2fsc[i])
                        elif abs(angle - 35.26) <= ang_tol_p or abs(angle - 144.74) <= ang_tol_p:
                            # octahedral angle
                            octahedral_2axes.append(ax)
                            octahedral_2axes_fsc.append(sorder2fsc[i])
                        elif abs(angle) < ang_tol_p or abs(180. - angle) < ang_tol_p:
                            # ax is same as 3fold
                            parellel_orders.append(2)
                            parellel_fscs.append(sorder2fsc[i])
                        elif abs(angle - 90.0) < ang_tol:
                ...
                # perpendicular axis
                if len(perpendicular_axes) > 0:
                    perp_ax_order = perpendicular_orders[np.argmax(
                        perpendicular_fscs)]
                    perp_axis = perpendicular_axes[np.argmax(
                        perpendicular_fscs)]
        ...
        if refinement_results is not None:
            ...
            if fsc_ax1_refined >= pg_decide_fsc:
                    ...
                    for j, ax2 in enumerate(sordernax):
                        if j > 0:
                            ...
                            emdalogger.log_string(
                                fobj,
                                '   Angle between them: % .3f' % angle)
                            if abs(angle) < ang_tol_p or abs(angle - 180.0) < ang_tol_p:
                ...
                if len(perpendicular_2fold_axes) > 0:
                    ...
                    perp_2fold_ax = perpendicular_2fold_axes[np.argmax(
                        perpendicular_2fold_fscs)]
                    refinement_results = refine_ax(
                        emmap1=emmap1,
        ...
        if len(sorder2ax) > 0:
            ...
            for i, ax in enumerate(sorder2ax):
                ...
                emdalogger.log_string(
                    ...
                    mainax_bestorder, mainax_bestoder_fsc))
                emdalogger.log_string(
                    fobj,
                    '   Axis2: %s   Order: %s   FSC: % .3f\n' %
                        (vec2string(ax), 2, sorder2fsc[i]))
            ...
            if len(perp_2ax) > 0:
                ...
                if mainax_bestorder == 2:
                    ...
                    if len(perpax_bestorder_list) > 0:
                        ...
                        perpax_refined = perpax_refined_list[np.argmax(
                                                perpax_bestorder_list)]                            
                        # choose pointgroup
                        pg = decide_pointgroup(
                            axeslist=[refined_mainax,perpax_refined],
                            orderlist=[mainax_bestorder,perp2ax_bestorder])[0]
                    else:
                        # no perp axis true
                        pg = decide_pointgroup(
                    ...
                    for pax in perp_2ax:
                        ...
                        if refinement_results is not None:
                            ...
                            if refined_perp_2ax_fsc >= pg_decide_fsc:
                                ...
                                perp2ax_bestorder = 2 # because mainax_order > 2
                                # choose pointgroup
                                pg = decide_pointgroup(
                                    axeslist=[refined_mainax,
                                            refined_perp_2ax],
    ...
    try:
        if len(sordernax) > 0:
            ...
            for i, ax in enumerate(sordernax):
                if i != mainax_number:
                    emdalogger.log_string(
                        ...
                        '   Axis1: %s   Order: %s   FSC: % .3f' %
                        (vec2string(refined_mainax), 
                        mainax_order, refined_mainax_fsc))
                    emdalogger.log_string(
                        fobj,
                        '   Axis2: %s   Order: %s   FSC: % .3f' %
                        (vec2string(ax), sordern[i], sordernfsc[i]))
            ...
            if len(par_nax_order) > 1:
                ...
                (mainax_bestorder, 
                mainax_bestoder_fsc) = get_axorder(
                    emmap1=emmap1,
                    refined_axis=refined_mainax,
