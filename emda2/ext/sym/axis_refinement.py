...
def rotmat_spherical_crd(phi, thet, angle):
    ...
    tm3 = np.array(
        [[a11, a12, a13], [a21, a22, a23], [a31, a32, a33]], "float"
    )
    ...
    def prep_data(self):
        ...
        if any(itm is None for itm in [self.bin_idx, self.nbin, self.res_arr]):
            if self.map_unit_cell is not None:
                self.pix = [
                    self.map_unit_cell[i] / sh
                    for i, sh in enumerate(self.map_dim)
                ]
                (
                    self.nbin,
                    self.res_arr,
...
class Bfgs:
    ...
    def get_wght(self):
        ...
        self.wgrid = fc.read_into_grid2(
            self.bin_idx, val_arr, self.nbin, nz, ny, nx
        )[:, :, :, 0]
        ...
        for i in range(3):
            df[i] = -np.sum(
                np.real(
                    ...
                    * (self.e1 * tpi * self.sv[i, :, :, :])
                )
            )
        dphi = dthet = 1 / (2 * nx)
        rm_f1, _, _ = rotmat_spherical_crd(
            phi=self.phi + dphi, thet=self.thet, angle=self.angle
        )
        rm_b1, _, _ = rotmat_spherical_crd(
            phi=self.phi - dphi, thet=self.thet, angle=self.angle
        )
        rm_f2, _, _ = rotmat_spherical_crd(
            phi=self.phi, thet=self.thet + dthet, angle=self.angle
        )
        rm_b2, _, _ = rotmat_spherical_crd(
            phi=self.phi, thet=self.thet - dthet, angle=self.angle
        )
        t1 = timer()
    ...
    def numeric_derivative(self, x, info):
        ...
        dphi = 1 / (2 * nx)  # 0.01
        phi = self.phi + dphi
        rotmat, _, _ = rotmat_spherical_crd(
            phi=phi, thet=self.thet, angle=self.angle
        )
        e1_dphi = self.st * get_f(
            self.e0, self.ereal_rgi, self.eimag_rgi, rotmat
        )
        ...
        rotmat, _, _ = rotmat_spherical_crd(
            phi=self.phi, thet=thet, angle=self.angle
        )
        e1_dthet = self.st * get_f(
            self.e0, self.ereal_rgi, self.eimag_rgi, rotmat
        )
    ...
    def derivatives(self, x, info):
        ...
        _, dR_dphi, dR_dtheta = rotmat_spherical_crd(
        ...
        for k in range(3):
            for l in range(3):
                ...
                temp1 = x1 * dR_dphi[k, l]
                a[k, l] = np.sum(
                    wgrid * np.real(self.e0 * np.conjugate(temp1))
                )
                temp2 = x1 * dR_dtheta[k, l]
                b[k, l] = np.sum(
                    wgrid * np.real(self.e0 * np.conjugate(temp2))
                )
        # print()
        # print(a)
        # print()
    ...
    def functional(self, x, info):
        ...
        self.e1 = (
            self.st
            * rotate_f(self.rotmat, self.e0, self.bin_idx, self.ibin)[
                :, :, :, 0
            ]
        )
        self.calc_fsc()
    ...
    try:
        for i in range(emmap1.ncycles):
            if i == 0:
                ...
                emdalogger.log_string(
                    fobj, "FSC between static and initial copy"
                )
                emdalogger.log_fsc(
                    fobj, {"Res.": emmap1.res_arr, "FSC": f1f2_fsc}
                )
                ...
                if emmap1.output_maps:
                    output_mapname = (
                        emmap1.emdbid
                        + "_rotated_ax_"
                        + combinestring(initial_axis)
                        + "_ang"
                        + str(round(ang, 2))
                        + ".mrc"
                    )
                    emdalogger.log_string(
                        fobj, "writing out %s" % output_mapname
                    )
                ...
                if ibin >= 5:
                    ...
                    emdalogger.log_string(
                        fobj,
                        "Fitting starts at %s (A)" % (emmap1.res_arr[ibin]),
                    )
                else:
                    print("Resolution= %s (A)" % emmap1.res_arr[ibin])
                    print(
                        "FSC between two copies is too low. FSC= %s ibin=%s"
                        % (f1f2_fsc[ibin], ibin)
                    )
            else:
                f1f2_fsc, frt = fsc_between_static_and_transfomed_map(
                ...
                if ibin_old == ibin:
                    ...
                    emdalogger.log_newline(fobj)
                    emdalogger.log_string(
                        fobj, "Plotting FSCs to %s" % plotname
                    )
                    ...
                    if emmap1.output_maps:
                        ...
                        transformedmap = np.real(
                            ifftshift(ifftn(ifftshift(frt)))
                        )
                    ...
                    emdalogger.log_string(
                        fobj,
                        "axis refinement went wrong! ibin_old=%i, ibin=%i"
                        % (ibin_old, ibin),
                    )
            ...
            if ibin >= 5:
                ...
                is_abandon = True
                emdalogger.log_string(
                    fobj, "ibin = %s, Cannot proceed axis refinement." % (ibin)
                )
                final_axis = initial_axis
                final_t = initial_t
                pos_ax = []
        ...
        if not is_abandon:
            if emmap1.com:
                pos_ax = [
                    (emmap1.com1[i] + final_t[i] * emmap1.map_dim[i])
                    * emmap1.pix[i]
                    for i in range(3)
                ]
            else:
...
def prepare_data_using_halfmaps(
    half1, imask, axis, symorder, resol=None, fobj=None
):
    import re
...
if __name__ == "__main__":
    ...
    prepare_data_using_halfmaps(
        half1=half1, imask=imask, axis=axis, symorder=symorder, resol=resol
    )
